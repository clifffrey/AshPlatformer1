...
const levels = [
  ...
  {
    name: "Level 3",
    message: "Level 3: final checkpoint. More shooters, tighter hazards, one last boss.",
    playerStart: { x: 54, y: 38 },
    platforms: [
      { x: 0, y: 508, w: 960, h: 32 },
      { x: 0, y: 0, w: 28, h: 508 },
    ...
    pressurePlates: [
      { x: 426, y: 422, w: 50, h: 8, triggered: false },
    ],
    wallShooters: [
      { x: 78, y: 395 },
      { x: 848, y: 262 },
...
function updatePressurePlates() {
  ...
  for (const plate of pressurePlates) {
    ...
    for (const shooter of wallShooters) spawnWallTrapProjectile(shooter);
  }
}
